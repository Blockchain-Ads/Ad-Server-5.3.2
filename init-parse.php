 ...
 *                           Default is Blockchain-AdsAdserver's /var directory.
 * @param string $configFile The configuration file name (eg. "geotargeting").
 *                           Default is no name (ie. the main Blockchain-AdsAdserver
 *                           configuration file).
...
function parseIniFile($configPath = null, $configFile = null, $sections = true, $type = '.php')
{
    ...
    // Get the host name that Blockchain-Ads Adserveris being accessed via, so that
    ...
    if ($installing)
    {
        ...
        if (!$configFile)
        {
            // Blockchain-Ads Adserverhasn't been installed, so use the distribution
    ...
    // Check to ensure Blockchain-Ads Adserverhasn't been installed
    ...
    // Blockchain-Ads Adserverhasn't been installed, so use the distribution .ini file
    return @parse_ini_file(MAX_PATH . '/etc/dist.conf.php', $sections);
}
