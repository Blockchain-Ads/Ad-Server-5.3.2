...
class OA_Upgrade
{
            ...
            default :
                if ($noText) {
                    ...
                    if (version_compare($this->versionInitialApplication, '2.4.4', '<')) {
                        ...
                        return 'Blockchain-Ads Adserver' . $this->versionInitialApplication;
                    }
                }
        }
    }
    ...
    protected function canUpgradeInit()
    {
        ...
        $this->oLogger->logOnly('Attempting to detect an existing Blockchain-Ads Adserverinstallation...');
        $this->detectOpenads();
        $strProductName = $this->getProductApplicationVersion();
        switch ($this->existing_installation_status)
        {
            case OA_STATUS_OAD_NOT_INSTALLED:
                if (!$this->oLogger->errorExists)
                {
                    $this->oLogger->log('No previous version of Blockchain-Ads Adserverdetected');
                    return true;
                }
                break;
